...
final TextEditingController _MaSanPham2Controller = TextEditingController();
...
final TextEditingController _loController = TextEditingController();
...
class frmKhoBTP_TP_KiemKeTonKho extends StatefulWidget {
  final MaSanPham;
  final ViTri;
  final Kho;
  final Lo;
  const frmKhoBTP_TP_KiemKeTonKho({Key? key,required this.MaSanPham,required this.ViTri,required this.Kho,required this.Lo}) : super(key: key);
  @override
  State<frmKhoBTP_TP_KiemKeTonKho> createState() => _frmKhoBTP_TP_KiemKeTonKhoState();
}
class _frmKhoBTP_TP_KiemKeTonKhoState extends State<frmKhoBTP_TP_KiemKeTonKho> {
  bool isChecked = false;
  @override
  void initState() {
    Clear();
    _maSanPhamController.text = widget.MaSanPham;
    _vitriController.text = widget.ViTri;
    _khoController.text = widget.Kho;
    _loController.text = widget.Lo;
    KiemTraTonKho(widget.MaSanPham,widget.ViTri,widget.Kho);
    super.initState();
  }
              ...
              Row(
                  ...
                  children: [
                    ...
                    Padding(
                      padding: const EdgeInsets.fromLTRB(0, 0, 0, 10),
                      child: SizedBox(
                        width: (MediaQuery.sizeOf(context).width - 30) * 0.4, height: 52,
                        child: TextField(
                          controller: _MaSanPham2Controller,
                          decoration: const InputDecoration(
                            border: OutlineInputBorder(
                              borderSide: BorderSide(color: Colors.grey, width: 1),
                              borderRadius: BorderRadius.all(Radius.circular(10)),
                          ...
                          //   if (upperCaseValue != _MaSanPham2Controller.text) {
                          //     // Cập nhật giá trị và giữ vị trí con trỏ
                          //     final selection = _MaSanPham2Controller.selection;
                          //     _MaSanPham2Controller.text = upperCaseValue;
                          //     _MaSanPham2Controller.selection = selection.copyWith(
                          ...
                          // },
                          inputFormatters: [
                            UpperCaseTextFormatter(), // Formatter để chuyển thành chữ hoa
                          ],
                        ),
                      ),
                    ),
                    Padding(
                      ...
                      child: SizedBox(
                        ...
                        child: InkWell(
                          ...
                          onTap: KiemKe_ChuyenDoiMaSanPham,
                    ...
                    Padding(
                      ...
                      child: SizedBox(
                        ...
                        child: TextField(
                          ...
                          // onChanged: (String value) {
                          //   final upperCaseValue = value.toUpperCase();
                          //   if (upperCaseValue != _vitri2Controller.text) {
                          //     // Cập nhật giá trị và giữ vị trí con trỏ
                          //     final selection = _vitri2Controller.selection;
                          //     _vitri2Controller.text = upperCaseValue;
                          //     _vitri2Controller.selection = selection.copyWith(
                          //       baseOffset: selection.baseOffset,
                          //       extentOffset: selection.extentOffset,
                          //     );
                          //   }
      ...
      if (barcode != null && barcode != '') {
        if (barcode.contains('/')) {
          ...
          if (barcode
              ...
              .length == 3)
            kho = barcode.split('/')[2];
          KiemTraTonKho(MaSanPham, vitri, kho);
        }
        else {
  ...
  Future KiemKe_ChuyenDoiMaSanPham() async {
    if(isChecked){
      ShowDialog.showAlertDialog(context, 'Mã Vật Liệu đã kiểm kê rồi !!!');return;
    }
    if(_MaSanPham2Controller.text.length != 10){
      ShowDialog.showAlertDialog(context, 'Mã Vật liệu không đúng , vui lòng xem lại');return;
    }
    tonkho?.maVatLieuChuyenDoi =  _MaSanPham2Controller.text.toUpperCase();
    tonkho?.soLuongKiemKe = double.parse(_soluongthucteController.text);
    tonkho?.idKhuVuc = 'LAN';
    ...
    if (barcode != ''  && ViTri != '' && ViTri != null  && Kho != '' && Kho != null ) {
      KhoNVLApi.TonKhoNVL_ViTri_GetMaVatLieu_GetViTri_GetKho(barcode,ViTri,Kho).then((value) async {
        if (value.isNotEmpty ) {
          ...
          _maSanPhamController.text = value[0].maVatLieu;
...
class UpperCaseTextFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(
    ...
    return newValue.copyWith(
      ...
      selection: newValue.selection, // Giữ vị trí con trỏ
    );
  }
}